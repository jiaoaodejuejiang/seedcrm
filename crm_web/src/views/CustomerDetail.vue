  ...
  <div class="stack-page" v-loading="loading">
    <section v-if="profile" class="metrics-row">
      <article class="metric-card">
        <span>客户名称</span>
        <strong>{{ profile.customer?.name || '未命名' }}</strong>
      ...
      <article class="metric-card">
        ...
        <small>电商绑定 {{ profile.customer?.ecomBindingCount || 0 }} 个</small>
      </article>
    </section>
    ...
    <section v-if="profile" class="panel">
      <div class="detail-grid">
        ...
        <article class="detail-card">
          ...
          <div class="chip-row">
            <el-tag
              v-for="tag in profile.tagDetails"
              :key="tag"
              effect="plain"
              type="success"
            >
        ...
        <article class="detail-card">
          <h3>企微状态</h3>
          <p>外部联系人：{{ profile.customer?.wecomExternalUserid || '--' }}</p>
          <p>企业微信员工：{{ profile.customer?.wecomUserId || '--' }}</p>
          <el-button
            size="small"
            type="primary"
            :disabled="!profile.customer?.wecomBound"
    ...
    <section v-if="profile" class="panel">
      ...
      <el-table :data="profile.orderHistory" stripe>
        ...
        <el-table-column label="操作" min-width="160">
          <template #default="{ row }">
            <el-button
              v-if="row.planOrderId"
              size="small"
              type="primary"
              @click="router.push(`/plan-orders/${row.planOrderId}`)"
            >
              进入服务页
            </el-button>
          </template>
    ...
    <section v-if="profile" class="panel">
      <div class="detail-grid">
        ...
        <article class="detail-card">
          ...
          <p v-else class="text-secondary">暂无触达记录</p>
        </article>
      </div>
    </section>

    <el-dialog v-model="messageDialogVisible" title="发送企微消息" width="480px">
      <el-input v-model="messageForm.message" type="textarea" :rows="4" placeholder="输入需要发送给客户的消息" />
      <template #footer>
